...
namespace AutoTool.Model.CommandDefinition
{
    ...
    public static class CommandRegistry
    {
        ...
        private static readonly ConcurrentDictionary<string, Type> _typeMapping = new();
        ...
        private static Dictionary<string, CommandInfo> InitializeCommands()
        {
            ...
            // 暫定的に直接コマンドを追加（属性の問題を回避）
            AddBasicCommands(commands);
            ...
            System.Diagnostics.Debug.WriteLine($"CommandRegistry: Found {commandTypes.Length} command types with attributes");
            ...
            foreach (var type in commandTypes)
            {
                try
                {
                    ...
                    foreach (var attrInstance in assembly.GetTypes().Where(t => t.GetCustomAttribute<CommandDefinitionAttribute>() != null))
                    {
                        var commandAttribute = attrInstance.GetCustomAttribute<CommandDefinitionAttribute>();
                        if (commandAttribute != null)
                        {
                            _typeMapping[commandAttribute.TypeName] = attrInstance;
                            _commandMapping[commandAttribute.TypeName] = (commandAttribute.CommandType, commandAttribute.SettingsType);
                        }
                    }
                }
                catch (Exception ex)
        ...
        /// <summary>
        /// 基本コマンドを手動で追加（暫定的な解決策）
        /// </summary>
        private static void AddBasicCommands(Dictionary<string, CommandInfo> commands)
        {
            // 基本的なコマンドを直接追加（すべてBasicカテゴリを使用）
            var basicCommands = new[]
            {
                (TypeName: "Click", CommandType: typeof(ClickCommand)),
                (TypeName: "Click_Image", CommandType: typeof(ClickImageCommand)),
                (TypeName: "Hotkey", CommandType: typeof(HotkeyCommand)),
                (TypeName: "Wait", CommandType: typeof(WaitCommand)),
                (TypeName: "Wait_Image", CommandType: typeof(WaitImageCommand)),
                (TypeName: "Loop", CommandType: typeof(LoopCommand)),
                (TypeName: "Loop_End", CommandType: typeof(LoopEndCommand)),
                (TypeName: "Loop_Break", CommandType: typeof(LoopBreakCommand)),
                (TypeName: "IF_ImageExist", CommandType: typeof(IfImageExistCommand)),
                (TypeName: "IF_ImageNotExist", CommandType: typeof(IfImageNotExistCommand)),
                (TypeName: "IF_End", CommandType: typeof(IfEndCommand)),
                (TypeName: "Execute", CommandType: typeof(ExecuteCommand)),
                (TypeName: "Screenshot", CommandType: typeof(ScreenshotCommand)),
                (TypeName: "SetVariable", CommandType: typeof(SetVariableCommand)),
            };

            foreach (var (typeName, commandType) in basicCommands)
            {
                try
                {
                    var itemType = typeof(AutoTool.Model.List.Type.BasicCommandItem);
                    var factory = CreateOptimizedFactory(itemType);
                    ...
                    var commandInfo = new CommandInfo
                    {
                        ...
                        SettingsType = typeof(object), // 暫定的にobjectを使用
                        Category = (CommandCategory)0, // 暫定的に0を使用
                        IsIfCommand = typeName.StartsWith("IF_") && typeName != "IF_End",
                        IsLoopCommand = typeName == "Loop",
                        ItemFactory = factory
                    };

                    commands[typeName] = commandInfo;
                    _typeMapping[typeName] = itemType;
                    _commandMapping[typeName] = (commandType, typeof(object));

                    System.Diagnostics.Debug.WriteLine($"CommandRegistry: Added manual command {typeName}");
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"CommandRegistry: Failed to add manual command {typeName}: {ex.Message}");
                }
            }
        }
        ...
        /// 型名から型へのマッピングを取得
        /// </summary>
        public static Dictionary<string, Type> GetTypeMapping()
        {
            Initialize();
            return new Dictionary<string, Type>(_typeMapping);
        }
    }
}
