...
- 待機時間・タイムアウト・検索間隔・リトライ間隔を、ミリ秒の単一入力ではなく「時間 / 分 / 秒」で編集できるように変更
- 1秒未満の細かい待機が必要な場合は、秒欄に `0.5` のような小数を入力できるように対応
- コマンド一覧の説明文でも、対象時間を `1分30秒` などの読みやすい表記で表示するように改善
- 起動、ファイル操作、マクロ実行開始/完了、標準 `ILogger` 出力を `Logs` 配下へ記録するように改善
- 配布版コマンド詳細と開発者向けコマンド開発ガイドを、時間入力の新しい編集方式に合わせて更新
