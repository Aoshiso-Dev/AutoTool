...
- `Esc` を約1.2秒長押し: 緊急停止（AutoTool が非アクティブでも有効）
- 実行中でも上部の実行ボタンは `停止` として押せます
...
- タイトルバー右側の `バージョン情報` ボタンから、アプリ名・バージョン・GitHub URL を確認できます
  - 表示される URL はクリックしてブラウザで開けます
...
- 実行前チェックパネル右上の `×` ボタンでパネルを閉じられます
- コマンドリストを操作（選択/追加/移動/削除/貼り付け）すると実行前チェックパネルは自動で閉じます
...
- 正式バージョンの正は GitHub タグ（`vMAJOR.MINOR.PATCH`）です
  - `deploy-to-c-autotool.ps1` で配布した場合、表示バージョンは最新タグに一致します
