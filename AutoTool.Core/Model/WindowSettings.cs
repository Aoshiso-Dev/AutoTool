...
public class WindowSettings
{
    ...
    private static readonly JsonSerializerOptions SerializerOptions = AutoToolJsonOptionsFactory.CreateWindowSettingsOptions();
    ...
    public void Save()
    {
        try
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[{_timeProvider.GetLocalNow():O}] WindowSettings saved: {SettingsFilePath}");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[{_timeProvider.GetLocalNow():O}] Failed to save WindowSettings: {ex.Message}");
        }
    }
    ...
    public static WindowSettings Load(TimeProvider? timeProvider = null)
    {
        ...
        try
        {
            if (File.Exists(SettingsFilePath))
            {
                ...
                if (settings is not null)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"[{resolvedTimeProvider.GetLocalNow():O}] WindowSettings loaded: {SettingsFilePath}");
                    return settings;
                }
            }
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[{resolvedTimeProvider.GetLocalNow():O}] Failed to load WindowSettings: {ex.Message}");
        }
